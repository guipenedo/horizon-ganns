...
class RFFMKeysDataset(Dataset):
    def __init__(self, root, one_hot_encode=False, add_noise_to_encoding=True):
        ...
        self.noise_gen = torch.distributions.Exponential(torch.full((batch_size, keys_size, MAX_KEYPRESSES + 1), NOISE_RATE).float())
...
rffm_data_full = RFFMKeysDataset(DATA_NORM_FULL_FILENAME, one_hot_encode=True)
...
class GANModel:
    ...
    def save_model(self):
        ...
        torch.save(data, MODEL_DATA_DIR + sep + self.model_data_filename)

    def load_model(self):
        data = torch.load(MODEL_DATA_DIR + sep + self.model_data_filename, map_location=DEVICE)
