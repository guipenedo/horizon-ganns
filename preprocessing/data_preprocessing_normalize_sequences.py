...
output_dir = "../processed_data/normalized_sequences"
    ...
    with open(os.path.join(raw_data_folder, filename), "r") as data_file:
        ...
        for index, row in df.iterrows():
            ...
            norm_data = normalize_csv_row(data_columns, [
                *get_columns_as_list(row, data_columns[:6]),
                # *get_diff_pos(row, prev_row),
                *bitmask_to_one_hot(row["forest_state"], 9),
                *get_columns_as_list(row, data_columns[15:19]),
        ...
        file_counter += 1
        if has_keys and len(file_data_norm) > 30:  # at least 30 seconds of data
            df = pd.DataFrame(file_data_norm, columns=data_columns)
            df.to_csv(output_dir + sep + filename, index=False)
