...
output_dir = f"processed_data/normalized_sequences_{NR_SECS}s"
os.mkdir(output_dir)
...
for filename in os.listdir(raw_data_folder):
    with open(os.path.join(raw_data_folder, filename), "r") as data_file:
        ...
        if has_keys:
            i = 0
            while (i + 1) * NR_SECS < len(file_data_norm):
                df = pd.DataFrame(file_data_norm[i*NR_SECS: (i + 1) * NR_SECS], columns=data_columns)
