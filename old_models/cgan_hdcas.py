...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.Sequential(
            # input layer
            *discriminator_block(keys_size + game_state_dim, d_dim, bn=False),
            # hidden layers
...
gan_model = GANModel(D, G, "cgan_hdcas_data.pt", torch.nn.BCELoss())


def train_d(real_keys, game_state, fake_game_state):
    d_optimizer.zero_grad()
    D_output = D(real_keys, game_state)
    # loss on real keys
    ...
    D_output = D(fake_keys, fake_game_state)
...
def train_model(num_epochs=40):
    ...
    for epoch in range(num_epochs):
        for batch_i, (real_keys, game_state) in enumerate(data_loader_keys):
            d_loss = train_d(real_keys, game_state, get_fake_game_state())
