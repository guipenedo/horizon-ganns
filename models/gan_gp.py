...
parser.add_argument("--lr_d", type=float, default=0.01, help="discriminator learning rate")
...
parser.add_argument("--lambda_gp", type=float, default=0.0015, help="loss weight for gradient penalty")
parser.add_argument("--n_critic", type=int, default=4, help="number of times to train discriminator per "
...
class Generator(nn.Module):
    def __init__(self, hidden_size, latent_dim, output_size):
    ...
    def forward(self, z):
        ...
        out = self.fc_layers(z)  # N x 1 x output_size
        return out
...
class Discriminator(nn.Module):
    ...
    def forward(self, future_action):
        return self.fc_layer(future_action)
...
def train(save=True):
    ...
    for fold, (train_ids, test_ids) in enumerate(folds):
        ...
        # Generator
        G = Generator(opt.g_hidden_size, opt.g_latent_dim, player_output_dim).to(DEVICE)

        # Discriminator
        D = Discriminator(opt.d_hidden_size, player_output_dim).to(DEVICE)
        ...
        for epoch in range(opt.num_epochs):
            ...
            for batch_i, data in enumerate(trainloader):
                ...
                with torch.no_grad():
                    ...
                    z = torch.randn((observed_keys.shape[0], 1, opt.g_latent_dim), device=DEVICE)

                    # predict
                    predicted_future_keys = G(z)

                # validity
                validity_gt = D(real_future_keys)
                validity_predicted = D(predicted_future_keys)

                # loss
                gradient_penalty = compute_gradient_penalty(D, real_future_keys, predicted_future_keys)
